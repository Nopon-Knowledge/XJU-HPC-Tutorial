...
这个作业申请 1 个 CPU 核和 2048 MB 内存，并直接执行一条 Python 测试命令，不依赖额外文件。

### 示例 2：提交通用单 GPU 作业

```bash
PYTHON="$HOME/venvs/my_project/bin/python"
PYTORCH_CHECK='import torch; print("PyTorch:", torch.__version__); print("CUDA:", torch.version.cuda); print("Available:", torch.cuda.is_available()); assert torch.cuda.is_available()'
...
```bash
PYTHON="$HOME/venvs/my_project/bin/python"
PYTORCH_CHECK='import torch; print("GPU:", torch.cuda.get_device_name(0)); assert torch.cuda.is_available()'
...
jsub -q normal -J hello-python \
  ...
  -e "$HOME/logs/hello-python.%J.err" \
  -n 1 -M 2048 \
  "$HOME/venvs/my_project/bin/python" -c "print('Hello from compute node')"
